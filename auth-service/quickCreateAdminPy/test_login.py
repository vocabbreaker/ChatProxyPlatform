    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if token:
                ...
                log_token(username, email, password, None, response.status_code, "Token not found in response")
        else:
            print(f"❌ Failed to log in as {username}: {response.text}")
            log_token(username, email, password, None, response.status_code, response.text)
    except requests.RequestException as e:
...
def main():
    ...
    with open(LOG_PATH, "w") as log_file:
        ...
        log_file.write("# Format: [timestamp] LOGIN STATUS: username='xxx', email='xxx', password='xxx'\n\n")
    ...
    print('curl -H "Authorization: Bearer <token>" http://localhost:3000/api/protected/me')
